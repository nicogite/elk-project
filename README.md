...
* Clone de ce projet sur le Workspace

* Téléchargement du fichier countries.csv

* Création d'un dossier dans logstash pendant le build de logstash :

"RUN mkdir /var/logstash_data"

* Ajout du fichier dans logstash via la création d'un volume :

"./logstash/data:/var/logstash_data"

* Ajout de la pipeline logstash logstash_world_countries.conf

* Noter qu'on définit un index dynamique pour accéder plus tard aux données : index => "countriesdata-%{+dd.MM.YYYY}"

* Dans le container logstash, on charge la pipeline :

"logstash -f pipeline/logstash_world_countries.conf"

=> ERROR : Logstash could not be started because there is already another instance using the configured data directory

* Les données devraient être accessibvle via l'API elasticsearch sur la route :
localhost:9200/countriesdata-30.05.2022
